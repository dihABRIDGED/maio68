...
try {
    switch ($_POST['action']) {
        case 'create':
            ...
            $stmt = $pdo->prepare("
                INSERT INTO contato_suporte (tipo, titulo, valor, icone, cor, ordem, ativo) 
                VALUES (?, ?, ?, ?, ?, ?, 1)
            ");
            $stmt->execute([$tipo, $titulo, $valor, $icone, $cor, $ordem]);
        ...
        case 'update':
            ...
            $stmt = $pdo->prepare("
                ...
                SET tipo = ?, titulo = ?, valor = ?, icone = ?, cor = ?, ordem = ? 
                WHERE id = ?
            ");
            $stmt->execute([$tipo, $titulo, $valor, $icone, $cor, $ordem, $id]);
